...
class BoxField:
    ...
    def _apply_distortion(self, xyz, vz):
        return return_xyz_formatted_array(*xyz.T, self.simbox.Lbox, self.simbox.cosmo, self.simbox.redshift,
...
class MockField:
    ...
    def make_rands(self, density_factor=None, seed=None):
        ...
        if self.cartesian_selection:
            ...
            if not seed is None:
                np.random.seed(seed)
            rands = (np.random.random((Nran, 3)).astype(np.float32) - 0.5) * self.get_shape(rdz=False)[None,
                                                                             :] + self.center[None, :]
...
class SimBox:
    ...
    def construct_model(self):
        ...
        self.model = empirical_models.PrebuiltHodModelFactory(self.hodname, redshift=self.redshift, cosmo=self.cosmo,
    ...
    def get_halo_occupation(self, galaxy_type="all"):
        ...
        if galaxy_type.lower() == "all":
            ...
            raise ValueError(
                f"galaxy_type={galaxy_type} not understood. Must be one of {'both', 'central', 'satellite'}")
...
class GalBox(SimBox):
    ...
    def __init__(self, halobox=None, **kwargs):
        ...
        if not self.empty and self.populate_on_instantiation:
            if self.populate_on_instantiation:
                self.populate_mock()
GalBox.__doc__ = HaloBox.__doc__ = SimBox.__doc__
...
#         return umcool.make_predictor_UMmags(self.halos, self.redshift, photbands)
#
# class UMField:
#     """
